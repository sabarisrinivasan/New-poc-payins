<script lang="ts">
interface PropType {
	class?: string;
	width?: string;
	height?: string;
}
let {height , width , class:className}:PropType = $props()
</script>

<svg
	{width}
	{height}
	class={className}
	viewBox="0 0 45 47"
	fill="none"
	xmlns="http://www.w3.org/2000/svg"
	xmlns:xlink="http://www.w3.org/1999/xlink"
>
	<mask
		id="mask0_927_2045"
		style="mask-type:luminance"
		maskUnits="userSpaceOnUse"
		x="23"
		y="10"
		width="20"
		height="26"
	>
		<mask
			id="mask1_927_2045"
			style="mask-type:luminance"
			maskUnits="userSpaceOnUse"
			x="23"
			y="10"
			width="20"
			height="26"
		>
			<path
				d="M23.9841 27.478L33.8373 10.4099L39.2047 13.5085C42.6695 15.5083 43.859 19.9422 41.856 23.4071L36.3071 33.0172C35.056 35.1823 32.288 35.9245 30.1196 34.6766L25.1379 31.8017C23.6276 30.9234 23.109 28.9884 23.9841 27.478Z"
				fill="white"
			/>
		</mask>
		<g mask="url(#mask1_927_2045)">
			<path d="M42.8302 7.00659H2.69141V41.0001H42.8302V7.00659Z" fill="white" />
		</g>
	</mask>
	<g mask="url(#mask0_927_2045)">
		<mask
			id="mask2_927_2045"
			style="mask-type:luminance"
			maskUnits="userSpaceOnUse"
			x="23"
			y="10"
			width="20"
			height="26"
		>
			<path d="M42.8286 10.3514H23.5371V35.3215H42.8286V10.3514Z" fill="white" />
		</mask>
		<g mask="url(#mask2_927_2045)">
			<rect
				width="20.5361"
				height="25.5146"
				transform="matrix(1 0 0 -1 23.209 35.5808)"
				fill="url(#pattern0_927_2045)"
			/>
		</g>
	</g>
	<path
		d="M22.9339 15.6281L10.6758 36.8579L16.0432 39.9564C19.508 41.9562 23.9419 40.77 25.9417 37.3052L33.8924 23.5334C35.1435 21.3683 34.4012 18.5971 32.2329 17.346L27.2512 14.471C25.7408 13.5959 23.8091 14.1145 22.9339 15.6281Z"
		fill="#FDBD00"
	/>
	<path
		d="M33.838 10.4067L30.0393 8.21562C25.7091 5.71343 20.1699 7.19789 17.6678 11.5313L10.6214 23.7312C9.37033 25.8963 10.1126 28.6675 12.2809 29.9186L16.0796 32.1096C18.2447 33.3607 21.0159 32.6185 22.267 30.4501L30.6779 15.881C32.4249 12.8538 36.2948 11.8198 39.3189 13.5668L33.838 10.4067Z"
		fill="#2DA94F"
	/>
	<mask
		id="mask3_927_2045"
		style="mask-type:luminance"
		maskUnits="userSpaceOnUse"
		x="3"
		y="12"
		width="18"
		height="29"
	>
		<mask
			id="mask4_927_2045"
			style="mask-type:luminance"
			maskUnits="userSpaceOnUse"
			x="3"
			y="12"
			width="18"
			height="29"
		>
			<path
				d="M18.7594 15.1064L14.5685 12.6917C12.7016 11.6156 10.3129 12.2541 9.23356 14.1178L4.20325 22.8074C1.72699 27.0858 3.19524 32.5569 7.48332 35.0267L10.6759 36.8644L14.5459 39.0944L16.2248 40.0602C13.2429 38.0637 12.2965 34.064 14.118 30.9169L15.421 28.6675L20.1888 20.4284C21.2681 18.568 20.6296 16.1824 18.7594 15.1064Z"
				fill="white"
			/>
		</mask>
		<g mask="url(#mask4_927_2045)">
			<path d="M42.8302 7.00659H2.69141V41.0001H42.8302V7.00659Z" fill="white" />
		</g>
	</mask>
	<g mask="url(#mask3_927_2045)">
		<mask
			id="mask5_927_2045"
			style="mask-type:luminance"
			maskUnits="userSpaceOnUse"
			x="3"
			y="12"
			width="18"
			height="29"
		>
			<path d="M20.7377 12.1406H3.00195V40.0666H20.7377V12.1406Z" fill="white" />
		</mask>
		<g mask="url(#mask5_927_2045)">
			<rect
				width="18.6692"
				height="28.315"
				transform="matrix(1 0 0 -1 2.78906 40.2358)"
				fill="url(#pattern1_927_2045)"
			/>
		</g>
	</g>
	<defs>
		<pattern id="pattern0_927_2045" patternContentUnits="objectBoundingBox" width="1" height="1">
			<use xlink:href="#image0_927_2045" transform="scale(0.0075188 0.00606061)" />
		</pattern>
		<pattern id="pattern1_927_2045" patternContentUnits="objectBoundingBox" width="1" height="1">
			<use xlink:href="#image1_927_2045" transform="scale(0.00826446 0.00546448)" />
		</pattern>
		<image
			id="image0_927_2045"
			width="133"
			height="165"
			preserveAspectRatio="none"
			xlink:href="data:image/jpeg;base64,"
		/>
		<image
			id="image1_927_2045"
			width="121"
			height="183"
			preserveAspectRatio="none"
			xlink:href="data:image/jpeg;base64,"
		/>
	</defs>
</svg>
